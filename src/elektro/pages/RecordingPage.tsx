...
import { useDetailRecordingQuery, useDetailSimulationQuery } from "../api/graphql";
    ...
    return (
      <ElektroRecording.ModelPage
        ...
        sidebars={
          <MultiSidebar
            map={{
              Comments: <ElektroSimulation.Komments object={data.recording.id} />,
            }}
          />
        }
      >
            ...
            <div className="col-span-11 h-32 p-3">
                  <div>
                    ...
                    <p className="mt-3 text-xl text-muted-foreground">
                      Duration: {data.recording.simulation.duration} ms dt: {data.recording.simulation.dt} s/ms 
                    </p>
            <Popover>
              <PopoverTrigger asChild>
            <Button variant="ghost" size="sm" onClick={() => setShow(!show)}>
                  <span className="text-xs">{!show ? "Show Model" : "Hide"}</span>
                </Button>
              </PopoverTrigger>
        ...
        <SimulationRender simulation={data.recording.simulation} hidden={hidden} hiddenStimuli={hiddenStimuli} highlight={[data.recording.id]}/>
            
         </div>
        <div className="flex-initial flex flex-row gap-2 mb-6">
          {data.recording.simulation.recordings.map((view, index) => (
            <Card
              className={cn("px-2 flex-1 cursor-pointer max-w-xs p-3", hidden.includes(view.id) && "opacity-20")}
              key={index}
              onClick={() => {
                setHidden((prev) =>
          ...
          {data.recording.simulation.stimuli.map((view, index) => (
            <Card
              className={cn("px-2 flex-1 cursor-pointer max-w-xs p-3", hiddenStimuli.includes(view.id) && "opacity-20")}
              key={index}
              onClick={() => {

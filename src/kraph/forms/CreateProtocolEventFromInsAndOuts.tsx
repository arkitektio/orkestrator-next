...
const inStructuresToReagentRoles = (
  ins?: Structure[],
): ReagentRoleDefinitionInput[] => {
  if (!ins) return [];
  return ins
    .filter((p) => p.identifier == "@kraph/reagentcategory")
    .map((p, index) => ({
      role: `in${index}`,
      label: `in${index}`,
      categoryDefinition: {
        categoryFilters: [p.object],
      },
    }));
};

const inStructuresToEntityRoles = (
  ins?: Structure[],
): ReagentRoleDefinitionInput[] => {
  if (!ins) return [];
  return ins
    .filter((p) => p.identifier == "@kraph/entitycategory")
    .map((p, index) => ({
      role: `in${index}`,
      label: `in${index}`,
      categoryDefinition: {
...
const outStructuresToReagentRoles = (
  ins?: Structure[],
): ReagentRoleDefinitionInput[] => {
  if (!ins) return [];
  return ins
...
const outStructuresToEntityRoles = (
  ins?: Structure[],
): ReagentRoleDefinitionInput[] => {
  if (!ins) return [];
  return ins
...
export const TForm = (props: { ins?: Structure[]; outs: Structure[] }) => {
  const [create] = useCreateProtocolEventCategoryMutation({});

  const firstEntity = props.ins
    ?.filter((x) => x.identifier == "@kraph/entitycategory")
    ?.at(0);

  if (!firstEntity) {
    return <div>Please provide at least one entity category as input.</div>;
  }
  ...
  const onUpdate = (input) => {
    create({
      variables: {
        input: {
          ...input,
          plateChildren: plateEditor.children,
          graph: data?.entityCategory?.graph.id || "",
        },
      },
  ...
  const { data } = useGetEntityCategoryQuery({
    variables: {
      id:
        props.ins?.filter((x) => x.identifier == "@kraph/entitycategory")?.at(0)
          ?.object || "",
    },
  });
  ...
  const myform = useForm<CreateProtocolEventCategoryMutationVariables["input"]>(
    {
      defaultValues: {
        ...
        sourceEntityRoles: inStructuresToEntityRoles(props.ins),
        targetEntityRoles: outStructuresToEntityRoles(props.outs),
        sourceReagentRoles: inStructuresToReagentRoles(props.ins),
        targetReagentRoles: outStructuresToReagentRoles(props.outs),
        plateChildren: initialValue,
        variableDefinitions: [],
      },
    },
  );

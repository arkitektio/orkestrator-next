...
import { useRequestAccessMutation, ZarrStoreFragment } from '@/mikro-next/api/graphql';
...
export const useCachedArray = (props: { store: ZarrStoreFragment }) => {
  ...
  const createCacheKey = (c: number, t: number, z: number, yStart: number, yStop: number, xStart: number, xStop: number) => {
    return `${props.store.id}_c${c}_t${t}_z${z}_y${yStart}-${yStop}_x${xStart}-${xStop}`;
  };

  // Update metadata and manage cache size
  const updateMetadata = useCallback(async (sizeChange: number) => {
    if (!db) return;
    
    const tx = db.transaction('metadata', 'readwrite');
    const meta = await tx.store.get('cacheInfo') || { lastAccessed: Date.now(), totalSize: 0 };

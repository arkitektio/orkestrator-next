...
export const Args = ({stream, onClick, id}: {
    id: number;
  ...
  onClick?: (instream: number, onposition: number) => void;
    ...
    {stream.length > 0 ? (
      stream?.map((s, index) => (
        ...
        <Tooltip delayDuration={100}><TooltipTrigger><Button
              variant="outline"
            ...
            </Button></TooltipTrigger><TooltipContent className="bg-background text-foreground border border-1 border-foreground">
                <p className="text-sm inline">
                {s?.identifier || s.kind}
                </p>
                <div className="text-xs mt-0"> {s?.description}</div>
                {s.scope == PortScope.Local && s.nullable == false && <div className="text-xs mt-0 text-muted-foreground"> Local non-nullable Ports cannot be constants</div>}

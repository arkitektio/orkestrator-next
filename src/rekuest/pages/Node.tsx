...
import { AssignationEventKind, DetailNodeFragment, useDetailNodeQuery } from "@/rekuest/api/graphql";
import { useNodeDescription, withRekuest } from "@jhnnsrs/rekuest-next";
...
export const DoNodeForm = ({node }: { node: DetailNodeFragment }) => {
  ...
  const yieldEvent = latestAssignation?.events.find(x => x.kind == AssignationEventKind.Yield)

  const errorEvent = latestAssignation?.events.find(x => x.kind == AssignationEventKind.Critical)
        ...
        <div className="rounded flex gap-2xl max-w-[80%] mt-2 gap-2">
          ...
          />
                ...
                <div className="flex-initial h-full flex flex-col ">
                  <Button type="submit" variant={"ghost"} className="my-auto block h-full">
                ...
                {yieldEvent ? <Card className="flex-1">
                    ...
                    <ReturnsContainer registry={registry} ports={node.returns} values={yieldEvent?.returns}>
                  ...
                  <Card className="flex-1">
                    ...
                    <CardContent>
                    <div className="flex flex-col gap-2">
                    {node?.returns?.map(p => <div>
                        <div className=" font-bold">{p.label || p.key}</div>
                        <div className="text-xs text-muted-foreground">{p.description}</div>
                      
                      
                      <div className="text-xs text-muted-foreground">{portToLabel(p)}</div>
                      </div>)}
                    </div>
                    </CardContent>
                </Card>
...
{errorEvent && <Card className="flex-1">
...
export default asDetailQueryRoute(
  ...
  ({ data, refetch }) => {
    ...
    const description = useNodeDescription({ description: data.node.description || ""});
    ...
    return (
      ...
      >
        <div className=" p-6">
            ...
            {data.node.testCases?.map((testCase, key) => <Card key={key}>
              <CardHeader>
                <CardTitle>
                  {testCase.name}
              ...
              <CardContent>
                {testCase.results?.map((result, key) => <div key={key}>
                  <RekuestTemplate.DetailLink object={result.template.id} className="font-bold">{result.template.interface} @ {result.template.agent.name}</RekuestTemplate.DetailLink>
                  <div>{result.passed ? <TiTick/> : <TbMedicalCross/>}</div>
                </div>)}
              </CardContent>
            </Card>)}
            </div>

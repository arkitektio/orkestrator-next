...
function schemaToJson(schema: zod.ZodTypeAny): any {
  if (schema instanceof zod.ZodObject) {
    ...
    return Object.fromEntries(
      Object.entries(shape).map(([key, value]) => [key, schemaToJson(value as zod.ZodTypeAny)])
    )
  }
  if (schema instanceof zod.ZodString) return { type: 'string', enum: schema._def.typeName }
  if (schema instanceof zod.ZodNumber) return { type: 'number', enum: schema._def.typeName }
...
export const MaterializedBlokContext = React.createContext<MaterializedBlokContext>({
  mblok: null as any
})
...
export const useMaterializedBlokContext = () => React.useContext(MaterializedBlokContext)
...
const buildUseRekuestState = <T extends MetaApplication<any, any>>(
  ...
  const hook = (state: keyof T['states']) => {
    ...
    if (!stateID) {
      console.error(`State ${String(state)} not found in materialized blok ${mblok.id}`)
      return {
        value: undefined,
        updatedAt: undefined,
...
const buildUseRekuestActions = <T extends MetaApplication<any, any>>(
  ...
  const hook = (action: keyof T['actions'], options?: UseActionOptions) => {
    ...
    const actionId = mblok.actionMappings.find((s) => s.key === action)?.implementation.id

...
export function detectPropertyChanges(
  ...
  if (plan.additions.length > 0 && plan.removals.length > 0) {
    ...
    plan.additions = plan.additions.filter((p) => !matchedRenames.has(p.key))
    plan.removals = plan.removals.filter((k) => !matchedRenames.has(k))
  }

  plan.needsMigration =
    plan.additions.length > 0 || plan.renames.length > 0 || plan.removals.length > 0
...
export function generateMigrationDescription(plan: MigrationPlan): string {
  ...
  if (plan.additions.length > 0) {
    parts.push(
      `Add ${plan.additions.length} new ${plan.additions.length === 1 ? 'property' : 'properties'}`
    )
  }

  if (plan.renames.length > 0) {
    parts.push(
      `Rename ${plan.renames.length} ${plan.renames.length === 1 ? 'property' : 'properties'}`
    )
  }

  if (plan.removals.length > 0) {
    parts.push(
      `Remove ${plan.removals.length} ${plan.removals.length === 1 ? 'property' : 'properties'}`
    )
  }

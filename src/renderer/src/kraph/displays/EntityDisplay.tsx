    ...
    return (
      <KraphEntity.DetailLink object={entity}>
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-xs text-muted-foreground shrink-0">{entity.category?.label ?? entity.label}</span>
        </div>
  ...
  return (
    <KraphEntity.DetailLink object={entity}>
      <div className="w-full rounded-lg border border-border/60 bg-card p-3 space-y-1">
        ...
        <div className="text-xs text-muted-foreground">{entity.category?.label ?? entity.label}</div>
      </div>
    </KraphEntity.DetailLink>
  );
};

 ...
 */
export class CachedStorage<Options = unknown> implements AsyncReadable<Options> {
  private storage: AsyncReadable<Options>
  protected memoryCache: Map<string, CacheEntry> = new Map()
  private pendingRequests: Map<string, Promise<Uint8Array | undefined>> = new Map()
  ...
  protected generateRangeCacheKey(key: AbsolutePath, range: RangeQuery, opts?: Options): string {
  ...
  private async getFromLocalStorage(cacheKey: string): Promise<Uint8Array | null> {
    try {
  ...
  private async setToLocalStorage(cacheKey: string, data: Uint8Array): Promise<void> {
    try {
  ...
  private async setToIndexedDB(cacheKey: string, data: Uint8Array): Promise<void> {
    try {
      const db = await this.openDB()
  ...
  private async setCachedData(cacheKey: string, data: Uint8Array): Promise<void> {
    const now = Date.now()
  ...
  async get(key: AbsolutePath, opts?: Options): Promise<Uint8Array | undefined> {

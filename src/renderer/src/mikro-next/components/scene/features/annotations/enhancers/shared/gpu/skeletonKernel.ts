...
import { tetTableWGSL } from "../../meshes/marchingTets";
import type { Vec3 } from "../strokeModel";

/**
 * The GPU skeleton kernels (compute-shader port of `enhancers/shared/`): the
 ...
 * unit-testable against the `enhancers/shared` reference without a GPU;
 ...
 * slider; `enhancers/shared/fieldSmooth.ts` is the CPU twin and documents the
 ...
 * (`enhancers/shared/tubeMarch.ts` is the CPU twin; the case table is
 * generated by `tetTable.tetTableWGSL()`, so the two cannot drift). One
 ...
 */
export const SKELETON_TUBE_WGSL = /* wgsl */ `
struct TubeParams {
  box_origin: vec3<i32>,
  capacity: u32,
  box_size: vec3<u32>,
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var t = 0u; t < 6u; t = t + 1u) {
    ...
    // NOTE: "from"/"to" are RESERVED keywords in WGSL — hence the names.
    ...
    for (var e = edge_begin; e < edge_end; e = e + 3u) {
      ...
      if (base + 3u > P.capacity) { continue; } // still counted: exact demand
      for (var k = 0u; k < 3u; k = k + 1u) {
        let edge = TET_TRI_EDGES[e + k];
        let p = edge & 7u;
        let q = edge >> 3u;
        let vp = values[p];
        let vq = values[q];
        let s = (P.iso - vp) / (vq - vp); // straddles iso: vq != vp
        let pos = cell + mix(corner_offset(p), corner_offset(q), s);
        vertices[base + k] = vec4<f32>(pos, 0.0);
      }
    }
  }
}

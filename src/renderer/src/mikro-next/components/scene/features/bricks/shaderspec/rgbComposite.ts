 ...
 *
 *   color = (norm_r, norm_g, norm_b),   norm = max(norm_r, norm_g, norm_b)
 *
 * with `norm_k = normalizeSlotValue(raw_k, window)` and NO gamma, invert,
 * opacity or LUT. `rgbComposite.test.ts` derives the same numbers by running
...
export function rgbSampleContribution(
  ...
  return { color: [r, g, b], norm: Math.max(r, g, b) };
}

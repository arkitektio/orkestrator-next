...
export type NetworkGpuBundle = {
  ...
  /** `capacity.nodes * 4` floats — (radius, ordinal, glyphScale, 0). */
  ...
  /** Flip `needsUpdate` on the three attributes after a pack. */
  markUploaded: () => void;
  dispose: () => void;
};
...
export function createNetworkGpuBundle(
  ...
  segmentMaterial.colorNode = Fn(() => {
    // The edge's ordinal is its START node's, as the CPU path always defined
    // it. `varying(...)` evaluates the storage reads in the VERTEX stage —
    // where `instanceIndex` is the true builtin — and the value is constant
    // across the quad, so interpolation is exact.
    const ordinal = varying(auxNode.element(edgesNode.element(instanceIndex.mul(2))).y);
    const base = TSL.materialColor;
  ...
  // depth cue, not lighting.
  const glyphColorNode = Fn(() => {
    const shade = max(normalView.z, float(0)).mul(0.45).add(0.55);
    return vec3(TSL.materialColor).mul(shade);
  });
  ...
  glyphMaterial.colorNode = glyphColorNode();
  ...
  arrowMaterial.colorNode = glyphColorNode();

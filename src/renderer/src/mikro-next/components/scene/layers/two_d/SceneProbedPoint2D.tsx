    ...
    <>
      {markerStates.map((markerState) => (
        <group key={markerState.key} matrix={markerState.affineMatrix} matrixAutoUpdate={false}>
          <group
            position={[
              markerState.markerPosition[0],
              markerState.markerPosition[1],
              markerState.markerPosition[2] + markerState.markerRadius * 0.2,
            ]}
            renderOrder={5}
          >
...
function resolveMarkerState(
  ...
  try {
    ...
    return {
      ...
      markerPosition,
      markerRadius: THREE.MathUtils.clamp(worldUnitsPerPixel * 8, minAxis * 0.006, minAxis * 0.04),
    };
  } catch {
    return null;
  }
